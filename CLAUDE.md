...
- `blog.html` + `<slug>.html` articles — **generated**; see "Journal content system"
...
```
npm run build:data     # py data/build_variants.py + npm run build:journal
```
Use the npm script, not `py data/build_variants.py` on its own: the journal's
"Shop this guide" cards bake in price, so the generator has to run after it.
...
## Journal content system (added 2026-08-10)

`blog.html`, every article page and **the whole of `sitemap.xml`** are generated.
Do not hand-edit them — each one opens with a "Generated by" comment and the
next build overwrites it.

```
content/site.yml                 hub copy, categories, sitemap page list
content/journal/<slug>.md        one file per article (front matter + body)
content/journal/_TEMPLATE.md     copy this to start a new article
scripts/build-journal.js         the generator
scripts/templates/journal/       article + hub templates, nav/footer partials
css/journal.css                  all article + hub styling (no inline <style>)
```

Publishing an article:

```powershell
# 1. copy content/journal/_TEMPLATE.md → content/journal/<slug>.md and write it
# 2. drop the hero into images/journal/  (JPEG, ~1536px wide, < 300 KB)
npm run build:journal          # writes <slug>.html, blog.html, sitemap.xml
npm run check:journal          # exits 1 if the committed output is stale
# 3. commit the .md AND the generated files, then push
```

What the build derives, so it never has to be maintained by hand: canonical /
description / OG / Twitter tags, JSON-LD (`BlogPosting` + `BreadcrumbList`,
plus `FAQPage` when the front matter has an `faq:` block), the hub listing with
category grouping, the related-articles block, `loading="lazy"` below the fold,
and the sitemap with `lastmod`.

Notes:
- **Re-run the build after `py data/build_variants.py`** — or just use
  `npm run build:data`, which chains the two. The "Shop this guide" cards bake
  in name/price/image from `data/product-data.json`; a stale build shows a
  stale price. An unknown SKU fails the build rather than rendering an empty
  card. This is enforced, not just documented: `check:journal` re-renders from
  the current `product-data.json` and diffs against the committed HTML, so a
  changed price shows up as `STALE`. It runs from `.git/hooks/pre-commit`
  (local, untracked) and from `.github/workflows/check-journal.yml` (on push
  to main and on PRs). Neither runs on Vercel — `buildCommand` is `null` and
  `.github/` is in `.vercelignore`.
- `sitemap.xml` `lastmod` comes from each file's last commit date (mtime when
  the file is dirty), articles from their own `published`/`updated`.
- Categories live in `content/site.yml`. An unknown `category:` fails the build.
- `content/` is in `.vercelignore` — sources are tracked but never served.
- The npm script is deliberately **not** called `build`: `vercel.json` sets
  `buildCommand: null`, and a script named `build` would invite Vercel to run
  one.
...
| `scripts/build-journal.js` | Journal + sitemap generator (`npm run build:journal`) |
| `content/journal/*.md` | Article sources — the only file you write to publish |
| `content/site.yml` | Journal categories, hub copy, sitemap page list |
| `css/journal.css` | Article + Journal hub styles |
...
# Regenerate product data after spreadsheet change (also rebuilds the journal)
npm run build:data
